  ...
  return (
    <Card className="glassmorphic">
      <CardHeader>
        ...
        <CardDescription>
          Personaliza y organiza los tipos de ingresos y gastos para que se ajusten a tu vida.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <CategoryFormDialog onSave={(cat) => addIncomeCategory(cat, incomeCategories.length)} categoryType="income" />
          </div>
              ...
              incomeCategories.map((cat, index) => (
                <div key={cat.id} className="flex justify-between items-center p-2 rounded-md hover:bg-muted/50">
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => reorderIncomeCategories(index, index - 1)} disabled={index === 0}>
                      <ArrowUp className="h-4 w-4" />
                    </Button>
                     <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => reorderIncomeCategories(index, index + 1)} disabled={index === incomeCategories.length - 1}>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold">Tipos de Gastos ➖💸</h3>
             <CategoryFormDialog onSave={(cat) => addExpenseCategory(cat, expenseCategories.length)} categoryType="expense" />
          </div>
          <div className="border rounded-lg p-4 space-y-2 neumorphic-inset">
             {isLoading ? (
              ...
              expenseCategories.map((cat, index) => (
                <div key={cat.id} className="flex justify-between items-center p-2 rounded-md hover:bg-muted/50">
                  <span className="font-medium">{cat.emoji} {cat.name}</span>
                  <div className="flex items-center gap-1">
                     <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => reorderExpenseCategories(index, index - 1)} disabled={index === 0}>
                        <ArrowUp className="h-4 w-4" />
                     </Button>
                     <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => reorderExpenseCategories(index, index + 1)} disabled={index === expenseCategories.length - 1}>
                        <ArrowDown className="h-4 w-4" />
                     </Button>
